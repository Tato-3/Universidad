...
a)  ¿Cuántos niveles de privilegio podemos definir en las estructuras de paginación? 
    ...
    Dos: nivel de usuario y nivel de superusuario
...
b)  ¿Cómo se traduce una dirección lógica en una dirección física? ¿Cómo
    ...
    mmu_mapPage(cr3, virtual, phy)
        ...
        PT = (cr3[directoryIdx] & ̃ 0xFFF)
        PT[tableIdx] = (phy & ̃ 0xFFF) | PAG_US | PAG_RW | PAG_P
        tlbflush()
...
- D = 1 cuando se escribe en la página.
- A = 1 cuando se escribe o lee en la página.
- PCD = 1 hace que la página se almacene en memoria rápida.
- PWT = 1 hace que al escribir la escritura se refleje en cache y memoria a la vez. Si PWT = 0, solo se actualiza la memoria cuando se desaloja la línea de caché.
- U/S = 1 indica si la página puede ser accedida por el espacio de usuarix. Si U/S = 0, solo puede ser accedido por supervisor/kernel.
- R/W = 1 indica si la página puede leerse y escribirse. Si R/W = 0, solo puede leerse.
- P = 1 indica si la página se encuentra cargada en memoria.
...
d)  ¿Qué sucede si los atributos U/S y R/W del directorio y de la tabla
    ...
    Page-Directory and Page-Table Protection* del manual 3 de Intel (pag 193)

Queda lo más restrictivo
...
Una página para el directorio de la tarea, una página para la tabla, y tres páginas para la memoria de la tarea (dos para el código y una para la pila de la tarea).
...
Es una caché que guarda traducciones (de dir virtual a dir física) ya hechas. 

Es necesario purgarlo pues al cambiar de tarea, por ejemplo, las direcciones guardadas no son válidas para la nueva tarea.

Cada traducción en la TLB posee los siguientes atributos:
- Dirección virtual
- Dirección física
- Bits de control

La entrada no se ve afectada en la tabla original.
...
a)  Escriban el código de las funciones `mmu_next_free_kernel_page`,
  ...
  Solo necesitamos la primer entrada del directorio.
...
b)  Completen el código de `copy_page`, ¿por qué es necesario mapear y
    ...
    Mapearla para poder acceder y desmapear para liberar recursos.

    Necesitamos mapear las páginas de destino y fuente porque el procesador accede a la memoria usando direcciones virtuales, y al mapear las páginas, el proceso puede acceder a las direcciones físicas correctas a través de las direcciones virtuales.
    Luego de hacer la copia, es importante desmapear las páginas para liberar los recursos y mantener la limpieza en la tabla de páginas, evitando así conflictos futuros o fugas de memoria.

    SRC_VIRT_PAGE es la dirección virtual de la página fuente y DST_VIRT_PAGE es la dirección virtual de la página destino.
    
    Necesitamos obtener el CR3 para poder mapear, pues el registro CR3 contiene la dirección base del directorio de páginas.
